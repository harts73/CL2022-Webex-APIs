...
class MyWindow(QMainWindow):
    ...
    def webex_qr(self):
        ...
        print(link)
        myqr = segno.make(link)
        out = io.BytesIO()
        print(type(out))

        myqr.save(out, kind="ppm", scale=10)
        myqr.save("someimage.ppm", kind="ppm", scale=10)

        print("Here")
        out.seek(0)
        time.sleep(2)
        pil_img = PIL.Image.open(out)
        #pil_img = PIL.Image.open("someimage.ppm")
        img = ImageQt.toqpixmap(pil_img)
        return img
    ...
    def webex_token(self):
        ...
        while not self.device_token:
            ...
            try:
                webex_token_request = requests.post(webex_token_url, headers=auth_headers, params=webex_token_params,
                                                    auth=(self.ci, self.cs))
            except:
    ...
    def get_recording_list(self):
        ...
        recordings_from = "2024-01-15T00:00:00+00:00"
        recordings_to = "2024-01-27T23:59:00+00:00"
